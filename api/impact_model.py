...
class ImpactModel:
    ...
    def thermal_exposure_Jpm2(self, r_m: float, luminous_eff=3e-3, energy_J: float | None = None,
        ...
        # Geometric spreading: use slant range in denominator if airburst,
        # but horizon occlusion depends on ground distance, not slant.
        if burst_alt_m is not None and burst_alt_m > 0.0:
            R = (r_m**2 + burst_alt_m**2) ** 0.5
        else:
            R = r_m
        f = self._horizon_fraction(r_m, Rf)
        return f * luminous_eff * E / (2.0 * pi * max(R, 1.0)**2)
    ...
    def distances_for_thermal_rings(self, luminous_eff=3e-3,
        ...
        def solve_for_r(target):
            # Allow sub-km solutions and detect unattainable thresholds.
            lo, hi = 1.0, 1_000_000.0
            def bisect_with_alt(alt):
                nonlocal lo, hi
                lo, hi = 1.0, 1_000_000.0
                # If even at 1 m the exposure is below target, no solution.
                if self.thermal_exposure_Jpm2(lo, luminous_eff, energy_J=E, burst_alt_m=alt) <= target:
                    return None
                for _ in range(70):
                    mid = 0.5*(lo+hi)
                    if self.thermal_exposure_Jpm2(mid, luminous_eff, energy_J=E, burst_alt_m=alt) > target:
                        lo = mid
                    else:
                        hi = mid
                return hi/1000.0

            # Try with provided burst altitude first.
            rkm = bisect_with_alt(burst_alt_m)
            if rkm is not None:
                return rkm
            # Fallback: if airburst altitude makes thresholds unattainable, approximate with surface-level radiating source.
            if burst_alt_m is not None and burst_alt_m > 0.0:
                rkm = bisect_with_alt(None)
                if rkm is not None:
                    return rkm
            return None
